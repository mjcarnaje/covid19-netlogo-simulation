globals [
  ...
  natural_death_count
  died_from_covid_count

  vaccinated_count

  peak_infection_rate
  current_infection_rate
  multi_peak_count
]
...
to go
  if all? turtles [status != "I"] [stop]

  ask turtles [
    update_turtle_variables
    move
    if status = "I" [
      spread
      recover_or_die
...
"" ""
PENS
"infected" 1.0 0 -2674135 true "" "plot (count turtles with [status = \"I\"] / count turtles)  * 100"
"recovered" 1.0 0 -10899396 true "" "plot (count turtles with [status = \"V\" or status = \"R\"] / count turtles)  * 100"
"susceptible" 1.0 0 -1184463 true "" "plot (count turtles with [status = \"S\"] / count turtles)  * 100"
"total" 1.0 0 -7500403 true "" "plot (count turtles  / count turtles)  * 100"
